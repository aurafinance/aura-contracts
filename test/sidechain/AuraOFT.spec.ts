...
import { AuraOFT, ProxyOFT, ERC20 } from "../../types/generated";
...
describe("AuraOFT", () => {
    ...
    describe("constructor", async () => {
        ...
        it("should properly store valid arguments", async () => {
            expect(await auraOFT.canonicalChainId(), "canonicalChainId").to.eq(canonicalChainId);
            // oFTCore
    ...
    describe("setConfig", async () => {
        ...
        it("sets configuration by selector", async () => {
            ...
            //   When  config is set.
            await auraOFT.connect(dao.signer)[SET_CONFIG_SELECTOR](canonicalChainId, lockSelector, lockConfig);
            // No events
            const newConfig = await auraOFT.configs(canonicalChainId, lockSelector);
        ...
        it("fails if caller is not the owner", async () => {
            await expect(
                auraOFT[SET_CONFIG_SELECTOR](canonicalChainId, "0xdd467064", {
                    adapterParams: "0x",
                    zroPaymentAddress: DEAD_ADDRESS,
                }),
                "fails due to ",
    ...
    describe("lock", async () => {
        ...
        it("fails if no fee is sent", async () => {
            ...
            // TODO - add some validation to the OFT
            await expect(auraOFT.lock(amount), "native fee").to.be.revertedWith(
                "LayerZeroMock: not enough native for fees",
            );
        });
        xit("fails if no fee is sent", async () => {
            await expect(auraOFT.lock(ZERO, { value: NATIVE_FEE }), "zero amount").to.be.revertedWith("Cannot stake 0");
        });
    });
});
